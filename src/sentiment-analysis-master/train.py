...
def train():    
    ...
    model = Doc2Vec(dm=0, min_count=0, window=10, vector_size=DIM_SIZE, hs=1, epochs=20, sample=0.0, negative=5, workers=5)
...
def get_srcs():
    ...
    for dat_file in dat_files:
        if "neg_" in dat_file: 
            ...
            pos_size += len(open(dat_file, encoding="utf8").readlines())  # A necessary evil for now
...
#def score():
    ...
    for count in range(ntrain_size+ntest_size):
        if count < ntrain_size:
            train_docvecs[count] = model.dv['NEG_' + str(count)]
            train_labels[count] = 0
        else:
            test_docvecs[count - ntrain_size] = model.dv['NEG_' + str(count)]
            test_labels[count - ntrain_size] = 0

    for count in range(ptrain_size+ptest_size):
        if count < ptrain_size:
            train_docvecs[ntrain_size + count] = model.dv['POS_' + str(count)]
            train_labels[ntrain_size + count] = 1
        else:
            test_docvecs[ntest_size + count - ptrain_size] = model.dv['POS_' + str(count)]
